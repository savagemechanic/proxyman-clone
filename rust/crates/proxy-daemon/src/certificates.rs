...
impl CertificateAuthority {
    pub async fn load_or_create(storage_dir: impl Into<PathBuf>) -> Result<Self> {
        ...
        let (ca_cert, ca_key, ca_cert_pem) = if cert_path.exists() && key_path.exists() {
            ...
            let cert = params.self_signed(&key).context("failed to reconstruct local CA")?;
            (cert, key, cert_pem)
        } else {
            ...
            let cert = params.self_signed(&key).context("failed to self-sign local CA")?;
    ...
    pub async fn identity_for_host(&self, host: &str) -> Result<Arc<LeafIdentity>> {
        ...
        self.inner.cache.write().await.insert(host.to_owned(), Arc::clone(&identity));
        Ok(identity)
    }
}
...
mod tests {
    ...
    async fn generates_distinct_leaf_identities_and_caches_by_host() {
        let dir = std::env::temp_dir().join(format!("proxyman-clone-ca-test-{}", std::process::id()));
